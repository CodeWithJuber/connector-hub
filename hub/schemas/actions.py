...
class StrictModel(BaseModel):
    ...
    def mutually_exclusive_options(self):
        ...
        for group in groups:
            supplied = [
                name for name in group if hasattr(self, name) and getattr(self, name) is not None
            ]
            if len(supplied) > 1:
...
PARAMS: dict[str, dict[str, str]] = {
    ...
    "hetzner": {
        "get_server power_on power_off reboot delete_server": "id",
        "create_server": (
            "name! server_type! image! location ssh_keys volumes networks user_data labels "
            "automount start_after_create placement_group datacenter firewalls"
        ),
    },
    "linode": {
        "get_linode boot shutdown reboot delete_linode": "id! config_id",
        "create_linode": (
            "region! type! image! root_pass! label authorized_keys authorized_users "
            "backups_enabled booted interfaces metadata placement_group stackscript_data "
            "stackscript_id tags group"
        ),
    },
    ...
    "contabo": {
        "get_instance start stop restart": "id instance_id",
        "create_instance": "image imageId image_id product productId product_id",
    },
    "whmcs": {
        "get_clients": "limit",
        "get_client": "client_id!",
        "get_invoices get_tickets": "limit status client_id",
        "reply_ticket": "ticket_id! message!",
        "add_client": (
            "firstname! lastname! email! password! companyname address1 city state postcode "
            "country phonenumber"
        ),
        "create_invoice": "client_id! items!",
        "get_products": "product_id group_id",
        "module_action": "service_id! action!",
    },
    "whm": {
        "create_account": "domain! username! password! plan contactemail",
        "suspend_account": "user! reason",
        "unsuspend_account terminate_account": "user! keepdns",
    },
    "cpanel": {
        "add_subdomain": "subdomain! rootdomain! dir!",
        "add_email": "email! password! quota",
        "create_database": "name!",
        "create_db_user": "name! password!",
        "file_list": "dir",
        "cron_add": "command! minute hour day month weekday",
    },
    "openai": {"chat": "messages! model temperature max_tokens", "embeddings": "input! model"},
    "kimi": {"chat": "messages! model"},
    "cloudflare_ai": {"chat": "messages! model", "run_model": "model! input!"},
    "claude": {"chat": "messages! model system max_tokens temperature"},
    "tawk": {
        "list_chats list_tickets": "property_id status",
        "get_chat": "chat_id!",
        "send_message": "chat_id! message!",
        "get_ticket": "ticket_id!",
        "reply_ticket": "ticket_id! message!",
        "list_agents property_info": "property_id",
    },
    "email": {
        "check_inbox": "account_id! limit",
        "send_email": "account_id! to! subject! body! html",
        "search": "account_id! query!",
    },
    "gmail": {
        "get_access_token": "label!",
        "send": "label! to! subject! body!",
        "list_messages": "label! query max_results",
        "get_message": "label! message_id!",
    },
    ...
    "ops_security": {
        "audit_password_strength": "password!",
        "check_ssl": "host! port",
        "scan_common_exposure": "host!",
        "ssh_config_audit": "path",
...
def register_connector_models(connector: Any) -> None:
    for action in connector.actions():
        ...
        model = create_model(
            f"{connector.name.title().replace('_', '')}{action.title().replace('_', '')}Request",
