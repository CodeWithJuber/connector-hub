...
class UpstreamError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        self.provider, self.category, self.status, self.retryable = (
            provider,
            category,
            status,
            retryable,
...
class SharedHttpClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_body_bytes, self.cache_entries, self.cache_ttl, self.attempts = (
            max_body_bytes,
            cache_entries,
            cache_ttl,
            attempts,
            ...
            try:
                ...
                try:
                    return max(
                        ...
                        min(
                            (
                                email.utils.parsedate_to_datetime(raw) - datetime.now(UTC)
                            ).total_seconds(),
                            60,
                        ),
                    )
        ...
        with self._semaphore(provider):
            for attempt in range(1, self.attempts + 1):
                ...
                try:
                        ...
                        for chunk in response.iter_bytes():
                            ...
                            if size > self.max_body_bytes:
                                raise UpstreamError(
                                    provider,
                                    "response_too_large",
                                    f"response exceeded {self.max_body_bytes} bytes",
                    ...
                    if safe_cache and not any(
                        k.lower() in {"authorization", "cookie", "x-api-key"} for k in headers
                    ):
                        entry = CacheEntry(
                            data,
                    ...
                    log.info(
                        ...
                        upstream_host=urlsplit(url).hostname,
                    )
                    return {
                        "ok": True,
                        "status": response.status_code,
                    ...
                    raise UpstreamError(
                        provider, category, str(exc), retryable=retryable_method
                    ) from exc
